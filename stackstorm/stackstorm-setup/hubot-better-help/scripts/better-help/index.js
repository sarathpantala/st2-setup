...
function InitHelp(robot, scripts, searcher) {
    ...
    robot.respond(/help(?:\s+(.*))?/i, (res) => {
        ...
        if (!res.match[1]) {
            const cmds = renameHelpCommands(allCommands, '');
        ...
        // hubot help me
        if (res.match[1] === 'me' || res.match[1].startsWith('me ')) {
            ...
            for (const k in scripts) {
                ...
                desc = desc.replace('hubot', '');
                reply.push(`* help ${k} - ${desc}  `);
            }
        ...
        if (selectedHelp && selectedHelp.commands.length > 0) {
            const cmds = renameHelpCommands(selectedHelp.commands.sort(), '');
        ...
        if (matches.length > 0) {
            const cmds = renameHelpCommands(matches, '');
